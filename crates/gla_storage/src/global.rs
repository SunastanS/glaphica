...
fn layout_from_spec(
    id: ImageId,
    spec: gla_ir::ImageLayoutSpec,
) -> Result<GlaImageLayout, GlobalStorageError> {
    GlaImageLayout::new(spec.width_px, spec.height_px).map_err(|source| {
...
mod tests {
    ...
    #[test]
    fn registry_patch_rejects_invalid_layout_specs_before_allocation() {
        let id = ImageId::new(1);
        let mut storage = new_storage_with_format(format());
        ...
        let err = storage
            .apply_registry_patch(RegistryPatch::new(vec![RegistryPatchOp::NewImage {
                id,
                format: format(),
                layout: ImageLayoutSpec::new(0, 1),
                role: ImageRole::Primitive,
            }]))
            .unwrap_err();
        ...
        assert!(matches!(
            ...
            GlobalStorageError::ImageCreate {
                id: err_id,
                source: ImageError::InvalidLayout {
                    source: ImageLayoutError::Empty
                }
            } if err_id == id
        ));
        assert!(storage.image(id).is_none());
    }
