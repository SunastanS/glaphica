...
impl Renderer {
    ...
    fn render_leaf_node_plan(
        ...
        let cache_key = LeafDrawCacheKey {
            layer_id,
            image_source,
        };
        ...
        if should_rebuild {
            ...
            if let Some(partial_tiles) = partial_tiles {
                ...
                cached_leaf.replace_all_instances(blend, image_source, full_instances);
            }
        }

        let (atlas_bind_group, pipelines) = match image_source {
            render_protocol::ImageSource::LayerImage { .. } => (
                self.gpu_state.atlas_bind_group_linear.clone(),
                self.gpu_state.composite_pipelines_rgba.clone(),
            ),
            render_protocol::ImageSource::BrushBuffer { .. } => (
                self.gpu_state.brush_buffer_atlas_bind_group_nearest.clone(),
                self.gpu_state.composite_pipelines_r32float.clone(),
            ),
        };
        let draw_context = DrawPassContext {
            target_view: context.target_view,
        ...
        match node {
            RenderTreeNode::Leaf {
                ...
                let should_rebuild = leaf_should_rebuild(
                    leaf_dirty_tiles,
                    self.cache_state.leaf_draw_cache.get(&cache_key),
                    *blend,
                    *image_source,
                );
