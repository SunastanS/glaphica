...
which records logical dirty tile slots and logical DrawOn passes. On flush,
Frame Handoff resolves those logical image tile slots through Session state,
materializes writable tile positions, emits first-write initialization before
DrawOn renderer work, and then uploads each DrawOn dirty set through the graph
and session command edges. The session records document dirty for `ReadWrite`
document ids and unions root demand for repaint. Expanded and matrix dirty
uploads currently fall back to full destination dirty.
...
frame dirty state and the logical DrawOn pass buffer. A typical frame is:

```text
1. App loop accepts input samples under FrameBudget.
2. For each accepted dab, DrawFrame records logical DrawOn work and frame dirty.
3. DrawFrame::flush performs Frame Handoff into Session, resolving logical image
   tile slots to writable tile positions.
4. Frame Handoff emits all first-write clears/copies before ordered DrawOn
   renderer work.
5. DrawFrame::flush uploads each DrawOn dirty set toward root.
6. DrawFrame::flush renders root demand, recursively materializing dependencies.
7. DrawFrame::flush submits DrawOn and derived passes to the render backend.
```
...
logical work was staged or Session state may already have been handed off. The
session is then marked aborted: later DrawOn calls and commit return an error,
and the caller must discard the session. This keeps unsubmitted GPU work from
becoming durable document state.
